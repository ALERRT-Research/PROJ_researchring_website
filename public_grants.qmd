...
grants <- yaml::read_yaml("_grants_ledger.yaml")

total_amount <- sum(map_dbl(grants, ~ .x$amount))
total_count  <- sum(map_int(grants, ~ if (!isTRUE(.x$subaward)) .x$n_awards else 0L))

fmt_amount <- paste0("$", formatC(total_amount, format = "d", big.mark = ","))
fmt_sub    <- paste0("across ", total_count, " awards · 2017–present")
```
...
<div class="rr-stat-number">`r fmt_amount`</div>
<div class="rr-stat-sub">`r fmt_sub`</div>
:::
...
### Extreme Risk Protection Orders and Firearm Suicide in New York: A Multi-State and Within-State Evaluation {#grant-rgvrc-2026}

::: {.rr-grant-meta}
**Funder:** Regional Gun Violence Research Consortium &nbsp;·&nbsp; **Year:** 2026 &nbsp;·&nbsp; **Amount:** \$49,999\
**PI:** Hunter Martaindale &nbsp;·&nbsp; **Co-PI:** Peter Tanksley
:::

#### Synopsis

Firearm suicide accounts for the majority of firearm deaths in the United States, yet rigorous evidence on whether Extreme Risk Protection Orders (ERPOs), civil court orders that temporarily remove firearm access from people at acute risk of harm, actually reduce it remains limited, and most prior work predates New York's law. This project evaluates New York's ERPO law through two complementary quasi-experimental analyses: a multi-state staggered difference-in-differences design estimating the effect of ERPO adoption on firearm suicide rates across all U.S. states from 2010 to 2024, and a within-New York fixed-effects panel testing whether counties with higher ERPO petition rates show lower firearm suicide rates across the state's 62 counties. Together the analyses assess both whether the law matters and whether how actively it is used matters, producing findings to inform New York legislators, the Division of Criminal Justice Services, and county-level implementers on ERPO resource allocation and potential expansion.

------------------------------------------------------------------------

### Preparing for Active Shooter Situations (PASS) {#grant-pass-2025}
...
### Preparing for Active Shooter Situations (PASS) {#grant-pass-hist}
...
### Assessing the effectiveness of door locks in school-based active shooter events {#grant-sia-2025}
...
[The Role of Locked Doors and Access Control in School-Based Active Shooter Events](public_reports.qmd#report-sia-2026)

------------------------------------------------------------------------

### ALERRT micro-learning video creation and evaluation {#grant-microlearning-2025}
...
### Integrated ICS training for all first responders {#grant-ics-2023}
...
### Fire as a Weapon (FAAW) In-Person and eLearning Training Opportunities {#grant-faaw-2021}
...
### Understanding the leakage that precedes mass public shootings and its utility for prevention efforts through public awareness {#grant-leakage-2024}
...
### Adapting Commercial VR-Based Training to Support AFSOC Mission Sustainment Team Protection & Security Training Objectives {#grant-vr-afsoc-2023}
...
### Firearm Detection Data {#grant-firearm-detection-2020}
